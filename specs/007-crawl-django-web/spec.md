...
- **중복 콘텐츠**: 동일한 URL에 대해 수집(Fetch)이 발생한 경우 항상 최신 내용으로 로컬 파일을 업데이트함.
...
- **FR-007**: 시스템은 수집(Fetch) 시 로컬 파일의 존재 여부와 관계없이 항상 최신 내용을 서버에서 가져와 덮어쓰기(Overwrite)를 수행하여 데이터의 최신성과 연결 구조의 완전성을 보장해야 함.
...
- **FR-017**: 시스템은 매 실행 시 전체 재귀 탐색 및 수집을 기본 동작으로 수행하며, 효율성을 위한 별도의 Resume(Skip) 기능은 제공하지 않음.
- **FR-018**: 시스템은 수집(Crawl) 또는 변환(Convert) 시작 전, 해당 출력 폴더(`temp` 또는 `data_sources`)의 기존 내용을 모두 삭제하여 원격지와의 완벽한 동기화(Sync) 상태를 보장해야 함. (단, 선택적 실행 시 해당 단계의 폴더만 비움)
