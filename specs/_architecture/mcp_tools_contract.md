...
에이전트가 Django 공식 가이드와 코드를 검색하기 위해 호출하는 RAG 기반 벡터 검색 도구입니다.
...
- `max_results` (선택, integer): 반환할 최대 청크 수 (기본값: 5).
...
- `relevance_score` (float): 벡터 유사도 또는 RRF 기반 검색 점수.
- `extra_meta` (object): 추가적인 메타데이터 (JSON).
