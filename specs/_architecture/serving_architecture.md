...
3.  **Django 검색 실행:** MCP 서버가 Django의 `HybridSearchService`를 직접 호출하여 다음 과정을 거칩니다.
    *   데이터베이스(`pgvector` + `pg_search`)를 통한 1차 고속 검색 (Vector + BM25)
    *   RRF 수식을 적용한 하이브리드 점수 통합 및 후보군 추출
    *   `RerankingService` 모델(ONNX)을 통한 최종 정밀 순위 정렬
4.  **최종 응답 반환:** 가공이 완료된 검색 결과(문서 청크 본문, 점수, 메타데이터)를
    MCP 프로토콜 표준 JSON 형식에 맞춰 에이전트에게 전달합니다.

---

## 4. 서버 성능 최적화 전략 (Performance Optimization)

*   **모델 선로딩 (Warm-up):** 서버 시작 시 임베딩 모델(`bge-m3`)과
    리랭커 모델(`keisuke-miyako/bge-reranker-base-onnx-int8`)을 메모리에 미리 로드(Load)하여,
    첫 번째 질의 시 발생할 수 있는 치명적인 지연 시간(Cold Start)을 방지합니다.
*   **비동기 처리 (Async Safety):** Django의 `sync_to_async` 데코레이터를 적극 활용합니다.
    이는 CPU 연산이 매우 무거운 리랭킹 과정이 MCP 서버의 메인 비동기 이벤트 루프(Event Loop)를
    차단(Block)하지 않도록 보호합니다.
*   **결과 캐싱 (Caching):** 반복적으로 들어오는 동일한 기술 질의에 대해서는
    이전 검색 결과를 메모리에 캐싱하여 데이터베이스 부하를 획기적으로 줄입니다.
