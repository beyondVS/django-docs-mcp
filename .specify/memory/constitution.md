<!--
Sync Impact Report:
- 헌법 버전: 1.8.0 (하이브리드 검색 및 Reranking 표준 추가)
- 수정 내용:
  - 2026-03-18: V. 품질 및 보안에 '하이브리드 검색 및 Reranking'을 RAG 품질 보증 필수 표준으로 명시.
  - 2026-03-18: III. 기술 표준에 pg_search(ParadeDB) 및 ONNX Runtime, Reranker 모델 정보 추가.
- 업데이트 확인 템플릿:
...
1. **기술 스택**:
   ...
   - Reranker: BAAI/bge-reranker-v2-m3 (ONNX INT8)
2. **테스트 관리 원칙**:
...
1. **RAG 품질 보증 (Hybrid Retrieval)**:
   ...
   - 골든 데이터셋(정답셋)을 통한 검색 정확도(Recall/MRR)를 주기적으로 검증하여 품질 퇴보(Regression)를 방지해야 합니다.
2. **데이터 수집 및 메타데이터 표준**:
...
**버전**: 1.8.0 | **비준일**: 2026-03-12 | **최종 수정일**: 2026-03-18
