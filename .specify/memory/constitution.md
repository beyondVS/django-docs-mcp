...
7. **아키텍처 문서의 지속적 진화 (Continuous Architecture Documentation)**: 시스템의 중대한 설계 결정, 신규 모듈 도입 또는 구조적 변화가 발생할 경우, 반드시 `specs/_architecture/` 디렉터리에 새로운 설계 문서를 추가하거나 기존 문서를 업데이트하여 시스템의 '살아있는 설계도'를 유지해야 합니다.
...
1. **기술 스택**:
   ...
   - Documentation Tools: **docutils** (RST Parsing), **markdownify** (HTML to MD Conversion)
2. **테스트 관리 원칙**:
...
2. **데이터 수집 및 메타데이터 표준**:
   ...
   - **공식 문서(RST) 수집 표준**: Sphinx 확장 구문을 포함하는 공식 문서는 `git sparse-checkout`을 통해 필요한 디렉토리만 효율적으로 수집하며, Sphinx 전용 Roles/Directives가 누락되지 않도록 정밀 변환 파이프라인(`rst_converter.py`)을 거쳐 저장해야 한다.
...
**버전**: 2.1.0 | **비준일**: 2026-03-12 | **최종 수정일**: 2026-03-23
